...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-ef9cc0a4";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-ef9cc0a4">'+'<span class="tile-face-ef9cc0a4 tile-front-ef9cc0a4"></span>'+'<span class="tile-face-ef9cc0a4 tile-back-ef9cc0a4"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ef9cc0a4")){return;}
var back=tile.querySelector(".tile-back-ef9cc0a4");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-ef9cc0a4");if(isMine){tile.classList.add("boom-ef9cc0a4");}
if(dim){tile.classList.add("dimmed-ef9cc0a4");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-ef9cc0a4")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-ef9cc0a4");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ef9cc0a4")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-ef9cc0a4");void panelEl.offsetWidth;panelEl.classList.add("jolt-ef9cc0a4");}
endRound();}
